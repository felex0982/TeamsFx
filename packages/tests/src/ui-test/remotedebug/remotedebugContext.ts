...
export async function configSpfxGlobleEnv() {
  try {
    console.log(`Start to set up globle environment:`);
    const result = await execAsync(
      "npm install gulp-cli yo @microsoft/generator-sharepoint --global"
    );
    console.log(`[Successfully] set up globle environment.`);
    console.log(`${result.stdout}`);
  } catch (error) {
    console.log(error);
    throw new Error(`Failed to set up globle environment: ${error}`);
  }
}
